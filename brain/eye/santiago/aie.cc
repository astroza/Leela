...
#define BINARY_THRESHOLD 90
	...
	bool forward_line(Navig *navig, Mat &src, vector<Point> &points) 
	{
		...
		Chicken::get_next_direction(navig, src, points, dir);
                ...
                if(rudder_angle >= 88 && rudder_angle <= 92)
                        rudder_value = 1.0;
                else if(rudder_angle < 88) {
                        rudder_value = 0.01 + (rudder_angle*1.1)/350.0;
                } else {
                        rudder_value = (rudder_angle-92)/(180-92)*100;
                }
